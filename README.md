...
Functional tests for OGX distribution — Bruno API contracts + Jupyter notebook integration tests. Runs against any OGX deployment (local podman, GH Actions CI, RHOAI cluster).
...
- name: Run functional tests
  ...
  run: ./scripts/run-tests-with-providers.sh
```
...
Compatible with `dorny/test-reporter` (GH Actions) and ReportPortal.
