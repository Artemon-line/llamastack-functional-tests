...
# Ensure the installed llama-stack-client matches the target server.
#
...
_client_version=$(uv run --project "${REPO_ROOT}" python3 -c "import llama_stack_client; print(llama_stack_client.__version__)" 2>/dev/null || echo "0.0.0")
...
echo "Updating pyproject.toml to require llama-stack-client~=${_server_major_minor}.0 ..."

# Update the version constraint in pyproject.toml
sed -i "s|\"llama-stack-client[^\"]*\"|\"llama-stack-client~=${_server_major_minor}.0\"|" "${PYPROJECT}"

# Re-sync the venv (updates uv.lock and installs the right version)
(cd "${REPO_ROOT}" && uv sync --quiet)

_new_version=$(uv run --project "${REPO_ROOT}" python3 -c "import llama_stack_client; print(llama_stack_client.__version__)" 2>/dev/null)
echo "Upgraded llama-stack-client: ${_client_version} -> ${_new_version}"
